...
export default function Index({ workoutSchedules, trainers, filters }) {
    ...
    const [trainerIdFilter, setTrainerIdFilter] = useState(filters?.trainer_id || "")
    const [statusFilter, setStatusFilter] = useState(filters?.status || "")

    const handleSearch = (value) => {
        setSearchValue(value)
        router.get(route("admin.workout-schedules.index"), {
            search: value,
            trainer_id: trainerIdFilter,
            status: statusFilter
        })
    }

    const handleTrainerFilter = (value) => {
        setTrainerIdFilter(value)
        router.get(route("admin.workout-schedules.index"), {
            search: searchValue,
            trainer_id: value,
            status: statusFilter
                    ...
                    <div className="p-8">
                        <div className="mb-8 flex items-center justify-between">
                            <div>
                                ...
                                <p className="mt-1 text-gray-600">Управление расписанием</p>
                            </div>
                            <button
                        ...
                        <WorkoutScheduleTable
                            ...
                            onEdit={(schedule) => {
                                setSelectedWorkoutSchedule(schedule)
                                setShowEditModal(true)
                            }}
                        />

                        <Pagination items={workoutSchedules} />
                    </div>
                </main>
            </div>
        </>
